...
app.post("/api/login", async (req, res) => {
  ...
  const hashed = hashPassword(password);  // Hash password using your hash function
  ...
  if (result.length > 0) {
    ...
    res.json({ message: "Logged in!", user: req.session.user });  // Include user data in response
  } else {
